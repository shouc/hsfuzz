...
PHP_RINIT_FUNCTION(mysqli)
{
    ...
    zval* file_path = zend_hash_str_find(Z_ARRVAL_P(carrier),
                                         "SCRIPT_FILENAME",
                                         sizeof("SCRIPT_FILENAME") - 1);
    char* file_path_s = Z_STRVAL_P(file_path);
    zval* cov_file_loc = zend_hash_str_find(Z_ARRVAL_P(carrier),
                                            "HTTP_COV_LOC",
                                            sizeof("HTTP_COV_LOC") - 1);

    char COV_FILE_NAME[1 << 12];
    if (cov_file_loc == NULL){
        time_t t = time(NULL);
        sprintf(COV_FILE_NAME, "%s/fuzz_sql/%ld-mysqli.txt", file_path_s, t);
    } else {
        sprintf(COV_FILE_NAME, "%s/fuzz_sql/%s-mysqli.txt", file_path_s, Z_STRVAL_P(cov_file_loc));
    }
    xsql_fp = fopen(COV_FILE_NAME, "w+");
	return SUCCESS;
}
/* }}} */
