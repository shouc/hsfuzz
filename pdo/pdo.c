...
PHP_RINIT_FUNCTION(pdo)
{
    ...
    if (cov_file_loc == NULL){
        time_t t = time(NULL);
        sprintf(COV_FILE_NAME, "%s/fuzz_sql/%ld.txt", file_path_s, t);
    } else {
        sprintf(COV_FILE_NAME, "%s/fuzz_sql/%s.txt", file_path_s, Z_STRVAL_P(cov_file_loc));
    }
    sql_fp = fopen(COV_FILE_NAME, "w+");
}
